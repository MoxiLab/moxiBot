...
function parseReversed(args) {
  const raw = Array.isArray(args) ? args.map(a => String(a || '').toLowerCase()) : [];
  return raw.includes('rev') || raw.includes('reversed') || raw.includes('invertida') || raw.includes('inversa');
}

function extractQuestion(args) {
  const raw = Array.isArray(args) ? args.map(a => String(a || '').trim()).filter(Boolean) : [];
  const filtered = raw.filter((t) => {
    const s = t.toLowerCase();
    return !(s === 'rev' || s === 'reversed' || s === 'invertida' || s === 'inversa');
  });
...
module.exports = {
  name: 'tarot',
  alias: ['tarot', 'carta', 'cartatarot'],
  Category: tarotCategory,
  usage: 'tarot [pregunta] [rev|invertida]',
  description: 'commands:CMD_TAROT_DESC',
  cooldown: 0,
  ...
  async execute(Moxi, message, args) {
    ...
    const reversed = parseReversed(args);
