...
Moxi.on("interactionCreate", async (interaction) => {
    if (interaction.isButton()) {
        ...
        if (typeof interaction.customId === 'string' && interaction.customId.endsWith('_d')) {
            ...
            return interaction.reply({ components: [buildV2Notice(msg)], flags: v2Flags() });
        }
        ...
        if (interaction.customId === "repit") {
            const lang = await moxi.guildLang(interaction.guild?.id, process.env.DEFAULT_LANG || 'es-ES');
            {
                ...
                if (guard) return interaction.reply(guard);
            }
        ...
        if (interaction.customId === "vol_up") {
            ...
            const player = Moxi.poru.players.get(interaction.guild.id);
            if (!player) {
                return interaction.reply({ components: [buildV2Notice(moxi.translate('MUSIC_NO_MUSIC_PLAYING', lang))], flags: v2Flags() });
            }
            const memberChannel = interaction.member.voice.channelId;
            const botChannel = interaction.guild.members.me.voice.channelId;
            if (!memberChannel || memberChannel !== botChannel) {
                return interaction.reply({ components: [buildV2Notice(moxi.translate('MUSIC_SAME_VOICE_CHANNEL', lang))], flags: v2Flags() });
            }
            const newVol = Math.min(player.volume + 10, 150);
            if (player.volume >= 150) {
                return interaction.reply({ components: [buildV2Notice(moxi.translate('MUSIC_VOLUME_MAX', lang))], flags: v2Flags() });
            }
            player.setVolume(newVol);
            await tryUpdateMainPanel(interaction, player, lang, moxi.translate('MUSIC_CURRENT_VOLUME', lang, { volume: newVol }));
            return interaction.reply({ components: [buildV2Notice(moxi.translate('MUSIC_VOLUME_UP', lang, { vol: newVol }))], flags: v2Flags() });
        }
        ...
        if (interaction.customId === "vol_down") {
            ...
            if (player.volume <= 0) {
                return interaction.reply({ components: [buildV2Notice(moxi.translate('MUSIC_VOLUME_MIN', lang))], flags: v2Flags() });
            }
            player.setVolume(newVol);
            await tryUpdateMainPanel(interaction, player, lang, moxi.translate('MUSIC_CURRENT_VOLUME', lang, { volume: newVol }));
            return interaction.reply({ components: [buildV2Notice(moxi.translate('MUSIC_VOLUME_DOWN', lang, { vol: newVol }))], flags: v2Flags() });
        }
