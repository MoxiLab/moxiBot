...
/**
 * Sync BOTSTATS_* i18n keys into all Languages/<locale>/misc.json files.
 *
 * - Uses Languages/en-US/misc.json as the source of truth.
 * - Adds only missing keys (does not overwrite existing translations).
 * - Preserves formatting by inserting new entries before the final closing brace.
 */
...
function getBotstatsKeys(sourceObj) {
    const keys = Object.keys(sourceObj).filter((k) => k.startsWith("BOTSTATS_"));
    keys.sort();
    return keys;
}
...
function syncFile(targetFilePath, sourceKeys, sourceObj) {
    ...
    const lines = missingKeys.map(
        (k) => `${indent}"${k}": ${JSON.stringify(sourceObj[k])}`,
    );
...
function main() {
    ...
    const sourceKeys = getBotstatsKeys(sourceObj);
    ...
    console.log(`Updated ${updated.length} files; added ${totalAdded} missing BOTSTATS_* keys.`);
    if (updated.length > 0) {
        console.log(
            "Updated locales:",
